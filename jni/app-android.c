...
void Java_foam_opensauces_Scheme_nativeInit(JNIEnv* env)
{
    app_alive = 1;
    sc=scheme_init_new();
    FILE *log_file=fopen("/sdcard/opensauces/starwisp-log.txt","w");
    if (log_file!=NULL) scheme_set_output_port_file(sc, log_file);
}

void Java_foam_opensauces_Scheme_nativeDone(JNIEnv* env)
{
    app_alive = 0;
    fclose(log_file);
}

jstring Java_foam_opensauces_Scheme_nativeEval(JNIEnv* env, jobject thiz, jstring code)
{
